...
const stakeCurve = async (claim = true) => {
    const lpAmount = await curveLPContract.methods.balanceOf(MY_ACCOUNT_ID).call();
    const lp_decimals = await curveLPContract.methods.decimals().call();
    const lp_fromWei = _.findKey(web3.utils.unitMap,
                            v => v == (new web3.utils.BN(10).pow(new web3.utils.BN(lp_decimals))).toString());

    console.log(`Currently holding ${web3.utils.fromWei(lpAmount, lp_fromWei)} tokens`);
    ...
    if(claim) {
        return new Promise((resolve, reject) => {
            curveGaugeContract.methods
                ...
                .send()
            .on('transactionHash', hash => {
                console.log(`Curve stake Tx hash: '${hash}'`);
            })
            .on('confirmation', (number, receipt) => {
                if(number == 0) { console.log(`Curve stake confirmation number: ${number}`); }
            })
            .on('receipt', receipt => {
                resolve(true);
            })
